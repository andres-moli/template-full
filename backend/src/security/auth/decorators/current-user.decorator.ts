...
export const CurrentUser = createParamDecorator( 
    ( validTypes:UserTypes[]=[], context: ExecutionContext ) =>{
        ...
        if(!isPublic){
            if(!user) 
                throw new InternalServerErrorException(`current user is undefined, check if CurrentPageVersionGuard is set to this controller`);
