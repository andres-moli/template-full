...
import { BadRequestException, CallHandler, ExecutionContext, ForbiddenException, Injectable, InternalServerErrorException, NestInterceptor } from '@nestjs/common';
...
export class UserStatusInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if(status === UserStatusTypes.PartlyActive) throw new ForbiddenException('The user does not have a complete record.');
