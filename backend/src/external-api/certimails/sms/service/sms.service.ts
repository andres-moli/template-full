...
import { ICertimailsDictionary } from '../../email/interface/email.interface';
...
export class SmsService {
  ...
  async createSms(smsArgs: SmsArgs): Promise<ISmsResponse>{
    ...
    const dictionary: ICertimailsDictionary = smsArgs.metadata ? JSON.parse(smsArgs.metadata) : {};
  ...
  private async __replaceHtmlWithDictionary(html: string, dictionary: ICertimailsDictionary) {
