...
import { And, Between, Brackets, In, LessThan, LessThanOrEqual, ILike, Like, MoreThan, MoreThanOrEqual, Not, Repository, SelectQueryBuilder } from "typeorm";
...
const conditions = {
    ...
    _startswith: (value) => ILike(value+'%'),
    _notstartswith: (value) => Not(ILike(value+'%')),
    _endswith: (value) => ILike('%'+value),
    _notendswith: (value) => Not(ILike('%'+value)),
    _contains: (value) => ILike('%'+value+'%'),
    _notcontains: (value) => Not(ILike('%'+value+'%')),
    _like: (value) => ILike(value),
    _notlike: (value) => Not(ILike(value)),
};
